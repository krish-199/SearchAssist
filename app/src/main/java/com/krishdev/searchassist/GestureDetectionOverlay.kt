...
class GestureDetectionOverlay(context: Context, private val windowManager: WindowManager) : View(context), ServiceSharedInstance.EnableOverlayListener {
    ...
    private val preferenceChangeListener = SharedPreferences.OnSharedPreferenceChangeListener { sharedPreferences: SharedPreferences, key ->
        when (key) {
            WIDTH_KEY, HEIGHT_KEY, HEIGHT_OFFSET_KEY -> {
                ...
                height = (sharedPreferences.getFloat("height", 0f) * heightPixelMultiplier).toInt()
                heightOffset = (sharedPreferences.getFloat("heightOffset", 0f) * heightPixelMultiplier ).toInt()
                Log.d("GestureDetectionOverlay", "Preferences changed: width=$width, height=$height, heightOffset=$heightOffset, heightPixel-$heightPixelMultiplier")
            }
        }
    }
    ...
    fun onCreate() {
            ...
            heightOffset = (sharedPreferences.getFloat(HEIGHT_OFFSET_KEY, 0f) * heightPixelMultiplier).toInt()
    ...
    private fun updateOverlays(width: Int = this.width, height: Int = this.height, heightOffset: Int = this.heightOffset) {
        ...
        if (::leftEdgeView.isInitialized && leftEdgeView.isAttachedToWindow) windowManager.updateViewLayout(leftEdgeView, touchLayoutLeft) else windowManager.addView(leftEdgeView, touchLayoutLeft)
        if (::rightEdgeView.isInitialized && rightEdgeView.isAttachedToWindow) windowManager.updateViewLayout(rightEdgeView, touchLayoutRight) else windowManager.addView(rightEdgeView, touchLayoutRight)

        Log.d("GDS", "Updating overlay")
    }

    private fun getWindowLayout(width: Int, height: Int, heightOffset: Int, isLeftView: Boolean = true): WindowManager.LayoutParams {
